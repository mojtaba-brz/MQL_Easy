...
public:
   ...
   bool                    close_all_specified_positions(int retry_limit);
   //-- Quick Access
...
double CPosition::GroupAverageOpenPrice()
  {
   ...
   if(sumPositionsTemp !=0)
      avgPriceTemp = utilsTemp.NormalizePrice(MathAbs(sumPriceTemp / sumPositionsTemp));
   return avgPriceTemp;
  }
...
double CPosition::GroupAverageVolume()
  {
   ...
   if(nVolumesTemp !=0)
      avgVolumesTemp = utilsTemp.NormalizeVolume(MathAbs(sumVolumesTemp / nVolumesTemp));
      ...
      if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES))
        {
         ...
         if(!this.ValidPosition(symbolTemp,magicTemp,typeTemp))
            continue;
...
int CPosition::get_total_specified_positions()
  {
   ...
   for(int i=0; i<OrdersTotal(); i++)
     {
      ulong ticket = OrderSelect(i, SELECT_BY_POS);
...
//|                                                                  |
//+------------------------------------------------------------------+
bool CPosition::close_all_specified_positions(int retry_limit = 100)
  {
