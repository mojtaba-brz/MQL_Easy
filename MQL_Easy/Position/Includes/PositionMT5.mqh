...
public:
   ...
   // this function is very sensitive to terminal ping
   // if you just entered a position before calling this
   // function, that position may not be accounted
   int               get_total_specified_positions();
   bool              close_all_specified_positions(int retry_limit = -1);
...
//-- check symbol
   ...
   if(!utilsTemp.SetSymbol(this.GetGroupSymbol()))
     {
      this.Error.Copy(utilsTemp.Error);
      return -1;
     }
   if(sumPositionsTemp !=0)
      avgPriceTemp = utilsTemp.NormalizePrice(sumPriceTemp / sumPositionsTemp);
   return avgPriceTemp;
  }
...
//-- check symbol
   ...
   if(sumVolumeTemp !=0)
      avgPriceTemp = utilsTemp.NormalizePrice(MathAbs(sumTemp / sumVolumeTemp));
   return avgPriceTemp;
  }
...
//-- check symbol
   ...
   if(nVolumesTemp !=0)
      avgVolumesTemp = utilsTemp.NormalizeVolume(sumVolumesTemp / nVolumesTemp);
      ...
      if(ticketTemp > 0)
        {
         ...
         if(!this.ValidPosition(symbolTemp,magicTemp,(int)typeTemp))
            continue;
         ...
         CUtilities utilsTemp;
         if(!utilsTemp.SetSymbol(symbolTemp))
           {
            this.Error.Copy(utilsTemp.Error);
            return;
           }
...
//|                                                                  |
//+------------------------------------------------------------------+
int CPosition::get_total_specified_positions()
  {
   int pos_counts = 0;
   for(int i=0; i<PositionsTotal(); i++)
     {
      ...
      if(magic == this.GroupMagicNumber && symbol == this.GroupSymbol)
        {
         pos_counts++;
        }
     }
...
//|                                                                  |
//+------------------------------------------------------------------+
bool CPosition::close_all_specified_positions(int retry_limit)
  {
   while(get_total_specified_positions())
     {
      for(int i=0; i<PositionsTotal(); i++)
        {
         ulong ticket = PositionGetTicket(i);
   ...
   return get_total_specified_positions() == 0;
  }

//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
int CPosition::get_position_type()
  {
   for(int i=0; i<PositionsTotal(); i++)
     {
      ulong ticket = PositionGetTicket(i);
      long magic = PositionGetInteger(POSITION_MAGIC);
      string symbol = PositionGetString(POSITION_SYMBOL);
      if(magic == GroupMagicNumber && symbol == GroupSymbol)
        {
...
//|                                                                  |
//+------------------------------------------------------------------+
double CPosition::get_current_profit()
  {
   for(int i=0; i<PositionsTotal(); i++)
     {
      ulong ticket = PositionGetTicket(i);
      long magic = PositionGetInteger(POSITION_MAGIC);
      string symbol = PositionGetString(POSITION_SYMBOL);
      if(ValidPosition(symbol, magic, 0))
        {
         SelectByTicket(ticket);
         return GetProfit();
        }
